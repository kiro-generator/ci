...
publish--;
dry -
  run` (could leak information)
- **NEVER** run `;
cargo;
install` (modifies global system state)
- **NEVER** run `;
cargo;
uninstall`
- **NEVER** run `;
...
- ✅ `;
cargo;
test` - Running tests (without `--;
release` for safety)
- ✅ `;
cargo;
build` - Building (prefer `--;
debug` over `--;
release`)
- ✅ `;
cargo;
...
cargo;
fmt--;
check` - Format checking (never with `--;
all` or without `--;
check`)
- ✅ `;
cargo;
doc--;
no -
  deps` - Documentation generation (local only)
- ✅ `;
cargo;
metadata` - Project metadata extraction

### Cargo Command Safeguards
- **ALWAYS** run cargo commands with `--;
locked` when possible to prevent dependency modifications
- **PREFER** `;
cargo;
check` over `;
cargo;
build` for faster validation
- **AVOID** `;
cargo;
build--;
release` unless necessary for performance-critical analysis
- **NEVER** modify `;
Cargo.toml` or `;
Cargo.lock` files
...
- **TEMPORARY**: You may create temporary files for analysis (in ` /
  tmp` or similar)
